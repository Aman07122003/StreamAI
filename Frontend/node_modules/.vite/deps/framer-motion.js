...
function attachSpring(value, source, options) {
  ...
  const startAnimation = () => {
    stopAnimation2();
    activeAnimation = new JSAnimation(__spreadProps(__spreadValues({
...
function startViewAnimation(builder) {
  ...
  css.commit();
  const transition = document.startViewTransition(() => __async(null, null, function* () {
    yield update();
  }));
    ...
    transition.ready.then(() => {
      ...
      targets.forEach((definition, target) => {
        for (const key of definitionNames) {
          ...
          for (let [valueName, valueKeyframes] of Object.entries(keyframes2)) {
            if (!valueKeyframes)
              continue;
            const valueOptions = __spreadValues(__spreadValues({}, getValueTransition(defaultOptions, valueName)), getValueTransition(options, valueName));
            ...
            const animation = new NativeAnimation(__spreadProps(__spreadValues({}, valueOptions), {
      ...
      for (const animation of generatedViewAnimations) {
        ...
        if (!targetDefinition) {
          ...
          let animationTransition = __spreadValues({}, getValueTransition(defaultOptions, transitionName));
...
var LayoutGroup = ({ children, id: id4, inherit = true }) => {
  ...
  const memoizedContext = (0, import_react14.useMemo)(() => __spreadProps(__spreadValues({}, context.current), { forceRender }), [key]);
...
function MotionConfig(_a) {
  ...
  config = __spreadValues(__spreadValues({}, (0, import_react17.useContext)(MotionConfigContext)), config);
...
function buildSVGAttrs(state, _a, isSVGTag2, transformTemplate, styleProp) {
  ...
  if (style2.transform || attrs.transformOrigin) {
    style2.transformOrigin = (_a2 = attrs.transformOrigin) != null ? _a2 : "50% 50%";
    delete attrs.transformOrigin;
  }
  if (style2.transform) {
    style2.transformBox = (_b2 = styleProp == null ? void 0 : styleProp.transformBox) != null ? _b2 : "fill-box";
...
function useRender(Component3, props, ref, { latestValues }, isStatic, forwardMotionProps = false) {
  ...
  const elementProps = Component3 !== import_react22.Fragment ? __spreadProps(__spreadValues(__spreadValues({}, filteredProps), visualProps), { ref }) : {};
  const { children } = props;
  const renderedChildren = (0, import_react22.useMemo)(() => isMotionValue(children) ? children.get() : children, [children]);
  return (0, import_react22.createElement)(Component3, __spreadProps(__spreadValues({}, elementProps), {
...
function createMotionComponent(Component3, { forwardMotionProps = false } = {}, preloadedFeatures, createVisualElement) {
  ...
  function MotionDOMComponent(props, externalRef) {
    let MeasureLayout2;
    const configAndProps = __spreadProps(__spreadValues(__spreadValues({}, (0, import_react27.useContext)(MotionConfigContext)), props), {
    ...
    return (0, import_jsx_runtime7.jsxs)(MotionContext.Provider, { value: context, children: [MeasureLayout2 && context.visualElement ? (0, import_jsx_runtime7.jsx)(MeasureLayout2, __spreadValues({ visualElement: context.visualElement }, configAndProps)) : null, useRender(Component3, props, useMotionRef(visualState, context.visualElement, externalRef), visualState, isStatic, forwardMotionProps)] });
  }
...
function MeasureLayout(props) {
  ...
  return (0, import_jsx_runtime8.jsx)(MeasureLayoutWithContext, __spreadProps(__spreadValues({}, props), { layoutGroup, switchLayoutGroup: (0, import_react29.useContext)(SwitchLayoutGroupContext), isPresent: isPresent2, safeToRemove }));
}
...
function scrollInfo(onScroll, _a = {}) {
  var _b = _a, { container = document.scrollingElement } = _b, options = __objRest(_b, ["container"]);
  if (!container)
    return noop;
...
function getTimeline(_a) {
  var _b = _a, { source, container } = _b, options = __objRest(_b, ["source", "container"]);
  var _a2, _b2, _c, _d;
  const { axis } = options;
  if (source)
    container = source;
  const containerCache = (_a2 = timelineCache.get(container)) != null ? _a2 : /* @__PURE__ */ new Map();
  timelineCache.set(container, containerCache);
  const targetKey = (_b2 = options.target) != null ? _b2 : "self";
  const targetCache = (_c = containerCache.get(targetKey)) != null ? _c : {};
  const axisKey = axis + ((_d = options.offset) != null ? _d : []).join(",");
  if (!targetCache[axisKey]) {
    targetCache[axisKey] = !options.target && supportsScrollTimeline() ? new ScrollTimeline({ source: container, axis }) : scrollTimelineFallback(__spreadValues({ container }, options));
  }
...
function scroll(onScroll, _a = {}) {
  var _b = _a, { axis = "y", container = document.scrollingElement } = _b, options = __objRest(_b, ["axis", "container"]);
  if (!container)
    return noop;
  const optionsWithDefaults = __spreadValues({ axis, container }, options);
...
function useScroll(_a = {}) {
  var _b = _a, { container, target } = _b, options = __objRest(_b, ["container", "target"]);
...
var MAX_REPEAT = 20;
function createAnimationsFromSequence(sequence, _a = {}, scope, generators) {
  ...
  for (let i = 0; i < sequence.length; i++) {
    ...
    const resolveValueSequence = (valueKeyframes, valueTransition, valueSequence, elementIndex = 0, numSubjects = 0) => {
      ...
      const _a2 = valueTransition, { delay: delay2 = 0, times = defaultOffset(valueKeyframesAsList), type = "keyframes", repeat, repeatType, repeatDelay = 0 } = _a2, remainingTransition = __objRest(_a2, ["delay", "times", "type", "repeat", "repeatType", "repeatDelay"]);
...
function getValueTransition2(transition, key) {
  return transition && transition[key] ? __spreadValues(__spreadValues({}, transition), transition[key]) : __spreadValues({}, transition);
}
...
function animateElements(elementOrSelector, keyframes2, options, scope) {
  ...
  for (let i = 0; i < numElements; i++) {
    ...
    const elementTransition = __spreadValues({}, options);
    ...
    for (const valueName in keyframes2) {
      ...
      const valueOptions = __spreadValues({}, getValueTransition(elementTransition, valueName));
      valueOptions.duration && (valueOptions.duration = secondsToMilliseconds(valueOptions.duration));
      valueOptions.delay && (valueOptions.delay = secondsToMilliseconds(valueOptions.delay));
      ...
      animationDefinitions.push({
        ...
        options: __spreadProps(__spreadValues({}, valueOptions), {
          element,
          name: valueName,
...
// node_modules/framer-motion/dist/es/components/Reorder/Group.mjs
function ReorderGroupComponent(_a, externalRef) {
  ...
  return (0, import_jsx_runtime10.jsx)(Component3, __spreadProps(__spreadValues({}, props), { ref: externalRef, ignoreStrict: true, children: (0, import_jsx_runtime10.jsx)(ReorderContext.Provider, { value: context, children }) }));
}
...
function ReorderItemComponent(_a, externalRef) {
  var _b = _a, { children, style: style2 = {}, value, as = "li", onDrag, layout: layout2 = true } = _b, props = __objRest(_b, ["children", "style", "value", "as", "onDrag", "layout"]);
  const Component3 = useConstant(() => motion[as]);
  ...
  return (0, import_jsx_runtime11.jsx)(Component3, __spreadProps(__spreadValues({ drag: axis }, props), { dragSnapToOrigin: true, style: __spreadProps(__spreadValues({}, style2), { x: point2.x, y: point2.y, zIndex }), layout: layout2, onDrag: (event, gesturePoint) => {
